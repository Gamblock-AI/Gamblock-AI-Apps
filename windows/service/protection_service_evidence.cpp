...
std::optional<Phase4Config> ReadPhase4Config() {
  const auto value = ReadTextFile(
      DataDirectory() / L"phase4-evidence" / L"config.json");
...
std::string ProtectionService::BeginPhase4Latency(
  ...
  if (pending_phase4_latency_.size() >= kMaximumPendingSamples) return {};
...
void ProtectionService::CompletePhase4Latency(
    const std::string& evidence_id) {
  PendingPhase4Latency sample;
  {
    std::lock_guard lock(phase4_evidence_mutex_);
    const auto found = pending_phase4_latency_.find(evidence_id);
    if (found == pending_phase4_latency_.end()) return;
    sample = found->second;
  ...
  const double relay_ms = std::max(
      0.0, sample.pre_input_duration_ms - sample.extraction_duration_ms);
  const double dispatch_to_visible_ms = std::max(
      0.0, input_to_visible_ms - sample.queue_duration_ms -
