...
| Pattern Interrupt | Implemented; Android source-side latency measurement recorded but unpromoted | Seven-second reduced-motion/offline UI, opaque per-intervention IDs, visible-frame acknowledgement, completion, duplicate coalescing, and browsing-data-free recovery handoff are wired. Android's resident native overlay owns the first committed frame, with Flutter retained only if Android rejects attachment; Windows now keeps a pre-warmed native shell and does not synchronously wait for browser navigation before presenting it. The Research staging build recorded 31 successful native samples with median `115.57 ms`, p95 `142.85 ms`, and zero block/visibility failures in a source-side measurement. It can become only a validated single-environment feasibility record after privacy-safe Phase 4 promotion. The current progress checkpoint remains pending a `researchRelease` Android + Chrome + `warm_foreground_online` capture. The former final-readiness latency matrix is replaced by the separate balanced local-model and cross-platform browser-support runtime contracts documented in the testing repository. |
...
represented by two separate client-runtime contracts: balanced local-model
evaluation and five-browser cross-platform regression. The actual product
