...
| Evaluation | Instrumented; Android latency and false-positive smoke evidence on one device | Hybrid/model/rule metrics, FPR slices, native latency, retention, and evidence completeness. Latency schema v2 and a local p95 gate are wired; the Research staging Android run on `22120RN86` passed with 31 successful samples and p95 `142.85 ms`. The same device also passed a manual false-positive smoke matrix covering the five user-provided safe domains plus eight additional safe URLs with numeric, opaque-token, query, coordinate, issue-number, version, and long-path shapes; this is smoke evidence, not a validated FPR slice. Model quality, formal FPR slices, retention, Windows latency, and broader device coverage remain unevaluated. Windows now has the matching low-latency path and deterministic model-only DOM fixture, but still requires Windows VM/device evidence. |

### Reproducible evaluation update (2026-08-31)

Phase 4 evidence now uses schema v3: opaque `browser_family` and `build_mode`
are mandatory in addition to run/device/scenario labels, artifact versions, and
duration fields. The browser family/build mode are allowlisted and no browsing
content is recorded. The gate remains p95 `input_to_visible_ms < 200 ms`, at
least 30 successful samples, and zero failed block/visibility outcomes per
platform/device/scenario/browser/build group.

The umbrella `evaluation/pkm-progress/` harness can replay the bounded passive
extension-to-Windows input contract and produce aggregate/hash-only evidence.
Its 2,592-row frozen HTML replay is a **provisional offline projection**, not
Android/Windows/browser runtime proof. The promoted deployment-aligned artifact
passes the selected numeric targets in that replay: accuracy 97.22%, precision
96.25%, recall 95.10%, F1 95.67%, and FPR 1.77%. This must not be described as
physical Android/Windows/browser runtime proof. The earlier Android 31-sample
latency statement remains documented-only until its JSONL export is available
to the schema validator. Windows VM/device evidence remains pending.

The capability-table entry for Hybrid-v2 predates the promoted
`gamblock-lr-14012bec0479` artifact. The current bundled pair contains 5,852
unigrams, 4,148 bigrams, a `0.80/0.20` fusion, and threshold `0.45`; the
paragraph above supersedes its historical artifact counts and parameters.

The ONNX-to-Hybrid-v2 exporter now verifies the actual vectorizer/scaler/
classifier layout and accepts any non-empty unary/bigram vocabulary width. This
permits a separately reviewed deployment-surface candidate to be exported
without relying on a historic vocabulary size; it does not copy that candidate
into app assets or bypass fixture, integrity, validation, final-test, or runtime
evidence gates.

The older capability-table wording that calls the dataset card and split
manifest absent is superseded by the checked-in model evidence files. Their
provenance and deployment parity remain incomplete, so the historical
full-content metrics and current deployment projection are still provisional.

Current-day aggregate snapshots are published by the authenticated Flutter
client with monotonic idempotency when protection events or app resume provide
an opportunity. This keeps local Flutter analytics and backend/website
analytics convergent before UTC day rollover; offline counters remain local
and retryable.
