...
| Cross-repository testing | Implemented tooling in separate repository; required Android browser matrix passed and recorded for six browsers; optional Windows runtime not run | Model replay, Phase 4 latency, Android anti-uninstall, and component-check evidence are owned by the public `Gamblock-AI-Testing` repository. This snapshot records client capability only and links to the canonical Flutter/Android report; the current claim/status mapping is maintained in the umbrella `context/progress-testing.md`. |

The active Android browser target now adds UPX Browser as a seventh required
cell. The six existing browser batches remain recorded; UPX remains pending
until its signed Research-release 5+5 batch is promoted by the testing
repository.
...
capture or OCR is requested. Runtime matrix status remains pending until a
signed Research-release 5+5 batch is recorded.
