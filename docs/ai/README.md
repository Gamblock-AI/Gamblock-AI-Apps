...
| Cross-repository testing | Implemented tooling in separate repository; runtime matrix pending | Model replay, Phase 4 latency, Android anti-uninstall, and component-check evidence are owned by the public `Gamblock-AI-Testing` repository. This snapshot records client capability only and links to the canonical Flutter/Android report; the PKM v5 claim/status mapping is maintained in the umbrella `context/progress-testing.md`. |

### Latency target update

The Phase 4 description in the capability table uses the current v3 safe
record shape, but its former all-group latency sentence is superseded by the
versioned targets in `gamblock-ai-testing/docs/config/targets.json`: one
homogeneous feasibility group; the PKM v5 `researchRelease` Android/Chrome
`warm_foreground_online` checkpoint (30 successful samples, no failure, p95
strictly below 200 ms); and the retained Android/Windows final matrix. The
