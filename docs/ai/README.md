...
### Android/OEM anti-uninstall limitation

The validated Redmi 12C `settings_uninstall` records are retained as failed
assertions because the expected `blocked` outcome was not observed, but their
interpretation is explicitly an Android/OEM platform limitation—not an
unresolved Flutter code defect. MIUI Settings can deactivate Device Admin and
continue the user-initiated uninstall; an ordinary application cannot veto
that OS-level action. The follow-up `redmi_settings_uninstall_fix_01` record
had the same outcome and is evidence of a retest, not a claim that a code fix
remains to be implemented. See the canonical
[`Flutter/Android report`](../../gamblock-ai-testing/flutter/report.md) and
the [anti-uninstall context](../../gamblock-ai-testing/docs/ai/android-anti-uninstall-context.md).
...
Samsung Internet and Xiaomi Browser are included in the required Android
browser-support matrix.
Samsung Internet 30.0.0.67 passed the physical Research-release browser matrix
batch: five gambling samples produced intervention and five non-gambling
samples were allowed. The batch includes the OCR-to-classifier-to-Pattern-
Interrupt path and is retained only as aggregate-safe evidence. The built-in
Xiaomi Browser also passed its five-gambling/five-non-gambling physical
Research-release batch with intervention and allow outcomes respectively.

### UPX Browser package routing

Research additionally routes the exact UPX Browser package
`net.upx.proxy.browser` through the standard local Chromium Accessibility
extractor. It is intentionally not treated as Samsung Internet, so no visual
capture or OCR is requested. Its signed Research-release 5+5 runtime matrix
cell is recorded as passed in the canonical Flutter/Android report.
...
Run-specific Android/Windows device identities, observed outcomes, and
privacy-safe aggregate records belong in the canonical testing repository.
This component context intentionally does not retain device serials, browser
fixtures, or run-specific pass/fail claims.
