...
| Pattern Interrupt | Implemented code path; runtime evidence pending | Seven-second reduced-motion/offline UI, opaque per-intervention IDs, visible-frame acknowledgement, completion, duplicate coalescing, and browsing-data-free recovery handoff are wired. Android's resident native overlay owns the first committed frame, with Flutter retained only if Android rejects attachment; Windows presents a native shell before the Flutter route takes over. Android device and Windows VM traces remain required. |
...
| Evaluation | Instrumented; not evaluated | Hybrid/model/rule metrics, FPR slices, native latency, retention, and evidence completeness. Latency schema v2 and a local p95 gate are wired, but no Android device or Windows VM capture has passed it. |
